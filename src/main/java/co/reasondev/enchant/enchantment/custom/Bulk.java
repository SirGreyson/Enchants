...
public class Bulk extends CustomEnchant {

    public Bulk(int id, String name, int maxLevel) {
    ...
    public void handleItemEquip(ArmorEquipEvent e) {
        ...
        if (hasEnchant(e.getNewArmorPiece())) {
            ...
            e.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.SLOW, Integer.MAX_VALUE, level > 2 ? 1 : level - 1));
            e.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, Integer.MAX_VALUE, level - 1));
        }
    }
